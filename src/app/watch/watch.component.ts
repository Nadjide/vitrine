...
export class WatchComponent implements OnInit {
  watches: Watch[] = [
    ...
    },
    {
      ...
      price: 20000,
      image:
        'https://pics.zeitauktion.com/2023/2300998_sw6_theme2_2k.jpg',
      date: new Date(),
      isFavorite: false,
    },
  ]
  ...
  toggleFavorite(watch: Watch): void {
    ...
    const favorites = this.watches.filter(w => w.isFavorite).map(w => w.name);
    localStorage.setItem('favorites', JSON.stringify(favorites));
  }

  handleSearchEvent(searchTerm: string): void {
    this.filteredWatches = this.watches.filter(watch => watch.name.toLowerCase().includes(searchTerm.toLowerCase()));
  }
}
